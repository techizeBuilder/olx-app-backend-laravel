...
class TipController extends Controller {
        ...
        try {
            ...
            foreach ($otherLanguages as $lang) {
                ...
                if (!empty($translatedDescription)) {
                    $translationData[] = [
                        'translatable_id'   => $tip->id,
                        'translatable_type' => Tip::class,
                        'key'               => 'description',
    ...
    public function edit($id) {
        ...
        $grouped = $tip->translations->groupBy('language_id');
        foreach ($grouped as $langId => $items) {
            $desc = $items->where('key', 'description')->first();
            $translations[$langId] = $desc?->value ?? '';
        }
